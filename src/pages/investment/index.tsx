...
import { Center, Box, VStack, Button, Text, Divider, Spacer, Flex, useDisclosure } from '@chakra-ui/react'
import { t } from '../../i18n'
...
export default function Investment() {
  ...
  const fetchData = async () => {
    ...
    const lock = Number(formatBalance(stBal)) + Number(formatBalance(jtBal)) + Number(formatBalance(traBal))
    ...
    const stakeLock = Number(formatBalance(stStake)) + Number(formatBalance(jtStake)) + Number(formatBalance(traStake))
      ...
      <VStack width='100%' minH='587px' p='24px' bgColor='white' >
        ...
        <Flex width='100%' height='200px' color='textHead' p={12}>
          <Box textAlign='center'>
            <Text fontSize='42px' fontWeight='bold'>{bals.apyST/10000}%</Text>
          ...
          <Spacer />
          <Box textAlign='center'>
            <Text fontSize='42px' fontWeight='bold'>{bals.apyJT/10000}%</Text>
          ...
          <Spacer />
          <Box textAlign='center'>
            <Text fontSize='42px' fontWeight='bold'>${bals.totalInvest}</Text>
        ...
        <Box width='100%' p='24px'>
          ...
          <Text fontSize={14} color='textDesc'>
            Holding senior token or investor token can get interest reward every day, and adding liquidity can get combo token
          </Text>
          <Center>
            <Button mr={8} w='280px' h='60px' mt='50px' colorScheme='grass' color='white' onClick={() => openModal('deposit')}> {t('deposit')} </Button>
            <Button w='280px' h='60px' mt='50px' colorScheme='reddish' color='white' onClick={() => openModal('withdraw')}> {t('withdraw')} </Button>
          </Center>
        </Box>
      </VStack>

      <ConvertModal {...modalProps} type={type} cb={fetchData} />
      <Center width='100%' textAlign='center' fontSize='16px' color='textStats' pt='24px' mb={4}>
        {/* <Spacer /> */}
