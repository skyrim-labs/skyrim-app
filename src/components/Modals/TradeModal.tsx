...
const ConvertModal = (props: ConvertProps) => {
  ...
  const fetchData = async () => {
    ...
    console.log('stInvest, jtInvest: ', formatBalance(stInvest), formatBalance(jtInvest))
    setBals({
  ...
  const renderOutput = (values: any) => {
    ...
    return <Flex color='textLabel'>
      <Text fontSize={12} fontWeight={400}>{output}</Text>
    </Flex>
  }
        ...
        <ModalBody py={6}>
          ...
          >
            {({ handleSubmit, values, errors, setFieldValue }) => {
              ...
              >
                <Box px={8} pt={4} borderRadius='lg' border="1px" borderColor="borderColor" >
                  {renderLable(values)}
                  <InputGroup>
                    <InputControl inputProps={{ fontSize: '24px', fontWeight: '600', size: 'lg', variant: 'unstyled', isDisabled: !isMint, value: !isMint ? values.token === 'senior' ? `${bals.stRedeem}` : `${bals.jtRedeem}` : values.amount }} name="amount" />
                    <InputRightElement >
                      {isMint && <Button mt='20px' h="40px" size="lg" variant='unstyled' _focus={{ boxShadow: 'none' }} onClick={() => {
                        setFieldValue('amount', values.token === 'senior' ? bals.stBal : bals.jtBal)
                      }}>
                        MAX
                      </Button>}
                    </InputRightElement>
                ...
                <RadioGroupControl mb={12} name="token" label="">
                  <Radio colorScheme={colorScheme} value="senior">{t('senior').toUpperCase() + ' token'}</Radio>
                  <Radio colorScheme={colorScheme} value="junior">{t('junior').toUpperCase() + ' token'}</Radio>
                </RadioGroupControl>
                {renderOutput(values)}
                <ApproveBtn {...btnStyle} token={values.token} contractAddr={getMainPoolContract()} >
                  <SubmitButton mt='20px' width='100%' h='60px' colorScheme={colorScheme}>{t(formType)}</SubmitButton>
                </ApproveBtn>
              </Box>
            }
            }
