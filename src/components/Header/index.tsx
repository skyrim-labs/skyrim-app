...
import { Box, Flex, Spacer, Button, Link, Image, Center, useColorMode, useColorModeValue, useDisclosure } from "@chakra-ui/react";
      ...
      <Flex h='80px' boxShadow="xl" bg="titleBg" >
        ...
        <Flex minW='300px' color='white'>
          <Center p='4' fontSize={14} >
            <Link ml={6} href="#feature">Product</Link>
            <Link ml={6} href="#token">Token</Link>
            <Link ml={6}>Docs</Link>
          ...
          <Center minW='200px'>
            {
              ethereum ? <Box >
                {address ? <Box><Button variant='outline' borderColor='primary' borderRadius="3xl" color='primary' onClick={() => showWalletModal()} >My wallet</Button></Box> :
                  <Button variant='outline' borderColor='primary' borderRadius="3xl" color='primary' onClick={() => getAccount()}>Connect to wallet</Button>}
              </Box> : <Box>Please install <Link>MetaMask</Link></Box>
            }
          </Center>
        </Flex>
