...
import { RadioGroupControl, NumberInputControl, InputControl, SubmitButton } from 'formik-chakra-ui'
import * as Yup from 'yup'
import { tokenApi, mainApi } from '../../utils/api'
import { toast, getMainPoolContract, formatBalance, getTokenContract } from '../../utils'
import { t } from '../../i18n'
...
const ConvertModal = (props: ConvertProps) => {
  ...
  const fetchData = async () => {
    ...
    setBals({
      jtBal: formatBalance(jtBal), stBal: formatBalance(stBal), daiBal: formatBalance(daiBal)
    })
  }
  ...
  const renderLable = (values: any) => {
    ...
    const bal = isDeposit ? bals.daiBal + ' DAI' : token === 'SENIOR' ? bals.stBal + ' ' + token : bals.jtBal + ' ' + token
    return (
          ...
          >
            {({ handleSubmit, values, errors, setFieldValue }) => (
              <Box maxWidth={'sm'} m='10px auto' as='form' onSubmit={handleSubmit as any}>
                ...
                <ApproveBtn {...btnStyle} token={isDeposit ? 'DAI' : values.token} contractAddr={getTokenContract(values.token)}>
                  <SubmitButton {...btnStyle} isLoading={loading} >{t(formType)}</SubmitButton>
                </ApproveBtn>
              </Box>
            )}
          </Formik>
