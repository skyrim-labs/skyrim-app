...
const WalletModal = (props: WalletProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setBals({
        jtBal: formatBalance(jtBal), stBal: formatBalance(stBal), traBal: formatBalance(traBal)
      })
    }
  ...
  >
    ...
    <ModalContent h='368px' w='446px'>
      <ModalHeader>
        <Text textAlign='center' h={45} color='textDesc' fontSize={20} fontWeight={500} >
          {t('wallet')}
        </Text>
      </ModalHeader>
      <ModalCloseButton />
      <ModalBody mt={10} pb={6}>
        <VStack >
          ...
          <Button w='100%' h='60px' colorScheme='grass' onClick={() => onClose()}>{t('Close')}</Button>
        </VStack>
      </ModalBody>
    </ModalContent>
  </Modal>
