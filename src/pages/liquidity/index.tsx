  ...
  const PoolCard = ({ token, tr, tvl, ttr, ptr }: any) => {
    ...
    const fetchData = async() => {
      ...
      setBals({
        ...
        totalRewardsToday: (Number(formatBalance(totalSupply)) * Number(formatBalance(rewardPerToken))).toString(),
      })
    }
    useEffect(() => {
      fetchData()
    ...
    return <Center minW='392px' background='white'>
      <VStack p={8} >
        ...
        <Button fontWeight={500} fontSize={16} h='60px' w='280px' colorScheme='grass' onClick={() => { handleOpen(token) }}>{t('enter')}</Button>
      </VStack>
    </Center >
  }

  const liquidityModal = pool && <LiquidityModal {...liqModalProps} pool={pool} />
